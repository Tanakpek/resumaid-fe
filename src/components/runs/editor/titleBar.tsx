...
export class TitleBar {
    ...
    addButton(iconClass, btnText, styles, id, tooltipText, isDropDown, items) {
        ...
        if (isDropDown) {
            // tslint:disable-next-line:max-line-length
            let dropButton = new DropDownButton({ select: this.onExportClick, items: items, iconCss: iconClass, cssClass: 'e-caret-hide', content: btnText, open: () => { this.setTooltipForPopup(); } }, button as any);
            return dropButton;
        }
        else {
